namespace BookShop
{
    ...
    public class StartUp
    {
        public static void Main()
        {
            ...
            .ForMember(dto => dto.AuthorName, opt => opt.MapFrom(b => $"{b.Author.FirstName} {b.Author.LastName}"))
            );

            var mapper = config.CreateMapper();

            GetBooksBooks(DBcontext, mapper);

        }
        public static void MapBooks(BookShopContext context, MapperConfiguration mapper)
        {
            BookDTO[] books = context
                .Books
                .ProjectTo<BookDTO>(mapper)
                .ToArray();
            foreach (var book in books)
            {
                Console.WriteLine($"{book.Title} {book.Price} {book.AuthorName}");
            }

        }
        public static void GetBooksBooks(BookShopContext context, IMapper mapper)
        {
            Book Book = context.Books.First();

            BookDTO bookDTO = mapper.Map<BookDTO>(Book);

            Console.WriteLine($"{bookDTO.Title} {bookDTO.Price} {bookDTO.AuthorName}");
        }
